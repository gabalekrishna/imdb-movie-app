...
export const FavoritesProvider = ({ children }) => {
    ...
    const fetchFavorites = async () => {
        try {
            const response = await fetch(`https://api.themoviedb.org/3/account/22243362/favorite/${type}?language=en-US&page=1&sort_by=created_at.asc?`,{
                headers: {
  ...
  console.log(type, "type from context")
    return (
        <FavoritesContext.Provider value={{favorites, setType, favoriteCounts}}>
            {children}
